...
const plugins = ['agentplugins-goal', 'caveman-installer', 'ponytail', 'pi-btw'];
...
for (const name of plugins) {
  ...
  const manifest = await tryTsConfig(pluginDir);
  if (!manifest) { console.log(`Skip ${name} — manifest not loaded`); continue; }
  
  const distDir = join(pluginDir, '.agentplugins-dist');
  console.log(`\n=== Recompile ${name} for ${targets.join(', ')} ===`);
  try {
    await compile({ manifest, targets, write: true, outDir: distDir, pluginRoot: pluginDir, silent: false });
    console.log(`Compiled OK`);
  } catch (e: any) { console.error(`Compile error: ${e.message}`); continue; }
