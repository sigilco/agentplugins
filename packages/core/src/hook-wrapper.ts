/**
 * AgentBridge Hook Wrapper Generator
 *
...
/**
 * AgentBridge Auto-Generated Hook Wrapper
 * Platform: ${platform}
 * Hook: ${hookName}
 * ID: ${wrapperId}
 *
...
const { handler } = require('./__agentbridge_handlers__.js');
...
export function generateHandlersModule(
  ...
  return `/**
 * AgentBridge Auto-Generated Handlers Module
 *
 * DO NOT EDIT — This file is regenerated on each build.
 */
...
async function handler(ctx) {
  const handlerFn = handlers[process.env.AGENTBRIDGE_HOOK_ID];
  if (!handlerFn) {
    throw new Error(\`Unknown hook ID: \${process.env.AGENTBRIDGE_HOOK_ID}\`);
  }
