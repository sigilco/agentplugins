...
3. **CLI Tool** — single `agentplugin build` command to generate all platform outputs
...
- Can integrate AgentPlugin adapter support into platform documentation

## v0.1.0 Scope

### Core Components
1. **@agentplugin/core** — Universal plugin manifest types and validation
2. **CLI** (`agentplugin` binary) — build, validate, and test plugins
...
- Interactive plugin scaffolding (`agentplugin init`)
...
### Functional
- [ ] `agentplugin build` successfully compiles example-logger to all 7 platforms
...
- Interactive `agentplugin init` scaffolding
- Enhanced CLI: `agentplugin lint`, `agentplugin preview`
...
### Adoption
- Number of plugins using AgentPlugin (target: 5+ by end of Q3 2026)
...
AgentPlugin removes the primary friction point in cross-platform plugin development. By providing a universal format + adapters pattern, we enable a thriving ecosystem of high-quality plugins that work seamlessly across AI agent platforms.
