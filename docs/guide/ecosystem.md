...
| [agentplugins-goal](https://github.com/sigilco/agentplugins-goal) | Autonomous goal-completion loop — set a goal once, the agent iterates until done across all Tier-1 harnesses. |
...
> **Note:** Tier-1 capability status per plugin lives in each plugin's README. For the harness-level baseline see the [compat matrix](/reference/compat-matrix).
